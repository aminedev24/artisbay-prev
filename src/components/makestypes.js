...
const Makestypes = () => {
  ...
  return (
    <div className="wrapper">
      <img
        className="title-img"
        src={`${process.env.PUBLIC_URL}/images/vehiclesearchtitle.png`}
        alt="vehicle quick search"
      />
          ...
          <div className="grid">
            {bodyTypes.map((type, index) => (
              <div
                className="grid-item"
                key={index}
                onClick={() => handleFilterChange(null, type.name)}
              >
          ...
          <div className="brands">
            {makes.map((make, index) => (
              <div
                className="brand"
                key={index}
                onClick={() => handleFilterChange(make.name, null)}
              >
                <img
                  src={`${
                    process.env.PUBLIC_URL
                  }/images/makes/${make.name.toLowerCase()}-logo.png`}
                  alt={make.name}
                  className="make-icon"
                />
                <p>{make.name}</p>
